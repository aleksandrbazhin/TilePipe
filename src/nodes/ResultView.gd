...
#	if tile.result_tiles_by_bitmask.empty():
#		return
	var tiles_by_bitmasks := tile.result_tiles_by_bitmask
...
#		rotated_texture_in_viewport.rect_size = new_viewport_size
	...
	var out_image_size: Vector2 = template_size * tile.output_tile_size
#	out_image_size += (template_size - Vector2.ONE) * output_tile_offset
	...
	for mask in tiles_by_bitmasks.keys():
		for tile_variant_index in range(tiles_by_bitmasks[mask].size()):
			var generated_tile: GeneratedTile = tiles_by_bitmasks[mask][tile_variant_index]
#			var tile_position: Vector2 = generated_tile.position_in_template * (tile_size + output_tile_offset)
			var tile_position: Vector2 = generated_tile.position_in_template * tile.output_tile_size
			if generated_tile.image == null:
				continue
			out_image.blit_rect(generated_tile.image, tile_rect, tile_position)
