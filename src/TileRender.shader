...
void fragment() {
	...
	if (out_x_plus) {
		if (out_y_plus) {
			...
			COLOR = get_color_from_rotated(UV, texture(TEXTURE, UV), overlay_texture_8, rotation_8, is_over, flip_x_8, flip_y_8);
		} else if (out_y_minus) {
			...
			COLOR = get_color_from_rotated(UV, texture(TEXTURE, UV), overlay_texture_2, rotation_2, is_over, flip_x_2, flip_y_2);
		} else {
			COLOR = get_color_from_rotated(UV, texture(TEXTURE, UV),  overlay_texture_4, rotation_4, plus_overlap > UV.x, flip_x_4, flip_y_4);
		}
	} else if (out_x_minus) {
		if (out_y_plus) {
			...
			COLOR = get_color_from_rotated(UV, texture(TEXTURE, UV), overlay_texture_32, rotation_32, is_over, flip_x_32, flip_y_32);
		} else if (out_y_minus) {
			...
			COLOR = get_color_from_rotated(UV, texture(TEXTURE, UV), overlay_texture_128, rotation_128, is_over, flip_x_128, flip_y_128);
		}
		 else {
			bool is_over = (minus_overlap < UV.x || ovelap_direction_64.x == 0.0);
			COLOR = get_color_from_rotated(UV, texture(TEXTURE, UV), overlay_texture_64, rotation_64, minus_overlap < UV.x, flip_x_64, flip_y_64);
		}
	} else {
		if (out_y_plus) {
			COLOR = get_color_from_rotated(UV, texture(TEXTURE, UV), overlay_texture_16, rotation_16, plus_overlap > UV.y, flip_x_16, flip_y_16);
		} else if (out_y_minus) {
			COLOR = get_color_from_rotated(UV, texture(TEXTURE, UV), overlay_texture_1, rotation_1, minus_overlap < UV.y, flip_x_1, flip_y_1);
		} else {
